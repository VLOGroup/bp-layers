...
from ops.lbp_semantic_pw.inference_op import Inference
from ops.lbp_semantic_pw.message_passing_op_cuda import MessagePassing
...
class BP(SubNetwork):
    ...
    def __init__(self, device, args, max_iter, num_labels, delta, mode_inference='expectation', mode_message_passing='min-sum', layer_idx=0, single_pw=False):
        super(BP, self).__init__(args, device)
        self.device = device
        self.max_iter = max_iter
        self.layer_idx = layer_idx
        self.delta = delta
        self.single_pw = single_pw
    ...
    def forward(self, prob_vol, edge_weights, jump):
        ...
        messages = torch.zeros((N, 4, H, W, K), requires_grad=True, device=self.device,
                               dtype=torch.float)

        # compute messages
        beliefs = self.message_passing.forward(prob_vol, edge_weights, messages, jump, self.single_pw)
