    ...
    int shared_mem_offset, unsigned short delta)
{
  ...
  if(delta >= old_version_threshold)
  {
    ...
    for(unsigned short label = 0; label < C; ++label)
    {
      ...
      float jump_cost = getJumpCost(c, label, jump, n, direction, y, x);

      float new_msg = sdata[label + blockDim.y * threadIdx.x] + jump_cost * edgeWeight;
  ...
  {
    float L2 = jump(n, direction, jump.size2 - 1, y, x);
    float offset = jump(n, direction, 0, y, x);

    unsigned short start = max(static_cast<unsigned short>(round(c - (delta - 1) + offset - 1)), 0); 
    unsigned short stop = min(static_cast<unsigned short>(round(c + delta + offset + 1)), C);
    ...
    msg = min_h + L2 * edgeWeight;
    msg_argmin = static_cast<int>(argmin_h); 

    sdata[tid] = h;
    __syncthreads();

    for(unsigned short label = start; label < stop; ++label)
    {
      // compute min in local var to avoid global mem writes
      float jump_cost = getJumpCost(c, label, jump, n, direction, y, x);
      float new_msg = sdata[label + blockDim.y * threadIdx.x] + jump_cost * edgeWeight;
  ...
  const unsigned int n)
{
    ...
    //int jump_idx = getJumpCostPos(c, min_index, jump.size2);
    float jump_cost = getJumpCost(c, min_index, jump, n, grad_xy_idx, y, x);
    ...
    //pairwise gradient
    addGradientJump(c, min_index, jump, n, direction, y, x, gradient_pairwise, grad_xy_idx, edgeWeight * additive_hor, edgeWeight * additive_up, edgeWeight * additive_down);

    //edge gradient
    atomicAdd(&gradient_edge(n, grad_xy_idx, y, x), jump_cost * additive_hor);
    atomicAdd(&gradient_edge(n, grad_xy_idx, y, x), jump_cost * additive_up);
...
namespace cuda
{
  std::vector<at::Tensor> lbp_reduction_min_sum(at::Tensor cost, at::Tensor jump, at::Tensor edge, at::Tensor messages, unsigned short delta)
  {
    ...
    if(numBlocksLR.y != 1)
      std::cout << "SOMETHING IS WRONG: Blocksize over disps is not 1" << std::endl;
                                                ...
                                                at::Tensor message_scale)
  {
    ...
    at::Tensor gradient_pairwise = at::zeros({1, 4, jump.size(2), H, W}, options);
